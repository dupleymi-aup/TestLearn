...
from app.services.data_service import (
    ...
    get_all_quizzes, get_quiz_by_id,
    get_questions_by_quiz, save_quiz_result,
...
def _process_question(question, form_data):
    ...
    if question.question_type == "multiple_choice":
        ...
        user_answer = all_values[0].strip() if all_values else ""
        expected = (question.expected_answer or "").strip().lower()
        if user_answer and user_answer.lower() == expected:
            return 1, user_answer
        return 0, user_answer
    ...
    if question.question_type == "multiple_choice":
        ...
        for i, pair in enumerate(question.matching_pairs or []):
            ...
            matching_results.append({
                "left": pair["left"], "correct_right": pair["right"],
                "user_right": user_match, "is_correct": is_pair_correct
            })
        return {
            "question": question.question_text,
            "correct_answer": "Все пары верно", "your_answer": "См. детали",
        ...
        for i, item_text in enumerate(correct_order):
            ...
            ordering_results.append({
                "item": item_text, "user_item": user_item,
                "position": i + 1, "is_correct": user_item == item_text
            })
        return {
            "question": question.question_text,
...
async def category_page(request: Request, slug: str):
    ...
    return templates.TemplateResponse(request, "category.html", {
        "category": category, "topics": topics, "quizzes": quizzes,
...
async def quiz_page(request: Request, quiz_id: int):
    ...
    return templates.TemplateResponse(request, "quiz.html", {
        "quiz": quiz, "questions": questions, "user": user,
        "csrf_token": generate_csrf_token(), "time_limit": time_limit
    })
...
async def quiz_result(request: Request, quiz_id: int, result_id: str):
    ...
    with get_db() as conn:
        cursor = conn.cursor()
        cursor.execute(
            "SELECT id, quiz_id, score, total, created_at, user_id FROM quiz_results WHERE id = ?",
            (result_id,)
        )
        row = cursor.fetchone()
        if not row:
            raise HTTPException(status_code=404, detail="Результат не найден")
        result = {"id": row[0], "quiz_id": row[1], "score": row[2], "total": row[3], "created_at": row[4], "user_id": row[5]}
    quiz = get_quiz_by_id(quiz_id)
    if not quiz:
        raise HTTPException(status_code=404, detail="Тест не найден")
    questions = get_questions_by_quiz(quiz_id)
...
async def feedback_submit(request: Request):
    ...
    create_feedback(name, message, email, rating)
...
async def profile_page(request: Request):
    ...
    with get_db() as conn:
        ...
        rows = cursor.fetchall()
        achievements = [
            {"name": r[1], "description": r[2], "icon": r[3] or "🏆", "earned_at": r[0]}
            for r in rows
        ]
        cursor.execute("""
        ...
        quiz_history = [
            {
                "id": r[0], "quiz_id": r[1], "score": r[2], "total": r[3],
                "created_at": r[4], "quiz_title": r[5],
    ...
    return templates.TemplateResponse(request, "profile.html", {
        "user": user, "user_stats": user_stats,
        "achievements": achievements, "quiz_history": quiz_history,
        "csrf_token": generate_csrf_token()
    })
