...
from app.services.data_service import (
    ...
    get_user_stats, get_user_achievements
)
...
async def login_submit(request: Request):
    ...
    if csrf_error:
        return templates.TemplateResponse(request, "login.html", {
            "csrf_token": generate_csrf_token(), "error": csrf_error
        })
    username = form.get("username", "").strip()
    password = form.get("password", "")
    if not username or not password:
        return templates.TemplateResponse(request, "login.html", {
            "csrf_token": generate_csrf_token(), "error": "Введите имя пользователя и пароль"
        })
    user = get_user_by_username(username)
    if not user or not verify_password(password, user.password_hash):
        return templates.TemplateResponse(request, "login.html", {
            "csrf_token": generate_csrf_token(), "error": "Неверное имя пользователя или пароль"
        })
...
async def register_page(request: Request):
    ...
    return templates.TemplateResponse(request, "register.html", {
        "csrf_token": csrf_token, "errors": {}, "form_data": {}
    })
...
async def register_submit(request: Request):
    ...
    if not csrf_token or not stored_token or not secrets.compare_digest(csrf_token, stored_token):
        return templates.TemplateResponse(request, "register.html", {
            "csrf_token": generate_csrf_token(),
            "errors": {"general": "Ошибка безопасности: неверный CSRF токен."},
            "form_data": {}
        })
    username = form.get("username", "").strip()
...
async def profile_page(request: Request):
    ...
    return templates.TemplateResponse(request, "profile.html", {
        ...
        "achievements": achievements, "csrf_token": generate_csrf_token()
    })
